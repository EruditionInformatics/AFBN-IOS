...
//  AFBN
//
//  Created by Erudition Informatics on 09/05/19.
...
class InvoicesViewController: UIViewController {
    ...
    var topContentArray = ["Invoice No.","Invoice Date/ Time","Quote No.","Member","Customer","SalesPerson","Total","Status"]
    ...
    */
    ...
    //MARK: Set invoices Scroll
    func setIvoicesScrollVw(){
        
        let heightPadding = (searchVw.frame.size.height + filterVw.frame.size.height + buttonVw.frame.size.height + 85.0 )
        ...
        mainScrlVw.contentSize = CGSize(width:(150.0 * CGFloat(8)) + 50.0, height: 50.0 * 51)
        mainScrlVw.isPagingEnabled = true
        mainScrlVw.isScrollEnabled = true
        mainScrlVw.backgroundColor = UIColor.white
        
        let topVw = UIView(frame: CGRect(x: 0.0, y: 0.0, width: (150.0 * 8) + 50.0, height: 50.0))
        topVw.backgroundColor = UIColor(red: 92.0/255.0, green: 142.0/255.0, blue: 91.0/255.0, alpha: 1.0)
        mainScrlVw.addSubview(topVw)
        
        let toplineVw = UIView(frame: CGRect(x: 0.0, y: 5.0, width: (150.0 * 8) + 50.0, height: 1.0))
        toplineVw.backgroundColor = UIColor.white
        topVw.addSubview(toplineVw)
        
        let bottomlineVw = UIView(frame: CGRect(x: 0.0, y: topVw.frame.size.height - 5.0, width: (150.0 * 8) + 50.0, height: 1.0))
        ...
        ticBtn.backgroundColor = UIColor.clear
        ...
        for i in 0..<topContentArray.count {
            ...
            let headerLbl = UILabel(frame: CGRect(x: xPad , y: 10.0, width: 150.0, height: 30.0))
        ...
        for i in 1...50{
            ...
            var yPad = 0.0
            if(i == 0){
                yPad = Double((firstHeight + 1.0) * Float(i))
            }else{
                yPad = Double(1.0 + 50.0 * Float(i))
            }
            
            let backVw = UIView(frame: CGRect(x: 0.0, y: yPad , width: (150.0 * 8) + 50.0, height: 50.0))
            backVw.backgroundColor = UIColor.white
            mainScrlVw.addSubview(backVw)
            
            let bottomlineVw1 = UIView(frame: CGRect(x: 0.0, y: backVw.frame.size.height - 5.0, width: (150.0 * 8) + 50.0, height: 1.0))
            ...
            ticBtn1.backgroundColor = UIColor.clear
            //ticBtn.addTarget(self, action: #selector(self.headerTicBtnAction(_:)), for: .touchUpInside)
            ...
            for j in 0..<topContentArray.count {
                ...
                let headerLbl1 = UILabel(frame: CGRect(x: xPad , y: 10.0, width: 150.0, height: 30.0))
                headerLbl1.text = String(describing: topContentArray[j])
                headerLbl1.backgroundColor = UIColor.clear
